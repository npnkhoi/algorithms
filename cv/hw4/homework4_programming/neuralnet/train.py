...
def training_step(model, X_batch, y_batch, reg):
    ...
    # names = model.parameters().keys()
    # grads = {name: 0 for name in names}
    
    scores, cache = model.forward(X_batch)
    loss, data_grads = softmax_loss(scores, y_batch)
    grads = model.backward(data_grads, cache)
    
    # regularize each weight matrix
    for name in ['fc1_w', 'fc2_w']:
        w = params[name]
        reg_loss, reg_grads = l2_regularization(w, reg)
        loss += reg_loss
        grads[name] += reg_grads
