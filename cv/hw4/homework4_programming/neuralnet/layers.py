...
def fc_backward(grad_out, cache):
    ...
    grad_x means dloss/dx = dloss/dout * dout/dx = gradients * (something)
    """
    x, w, b = cache
    ...
    ###########################################################################
    grad_x = grad_out @ w.T # (N, Dout) * (Din, Dout)T -> (N, Din)
    grad_w = x.T @ grad_out # (N, Din)T * (N, Dout) -> (Din, Dout)
    N = x.shape[0]
    grad_b = np.squeeze(np.ones(shape=(1, N)) @ grad_out, axis=0) # (1, N) * (N, Dout) -> (1*, Dout)
...
def relu_backward(grad_out, cache):
    ...
    - cache: Input x, of same shape as grad_out
...
def softmax_loss(x, y):
    ...
    E = np.exp(x - M) # (N, C)
    marginal = np.sum(E, axis=1, keepdims=True) # (N, 1)
    p = E / marginal
    # Cross entropy loss
    loss = -(sum([np.log(p[i, y[i]]) for i in range(N)])) / N
    # Backward
