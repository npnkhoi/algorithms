...
def backproject(depth, intrinsic_matrix):
    ...
    # Create homogeneous coordinates: stack [u, v, 1] for each pixel
    ones = np.ones((H, W))
    coords = np.stack([u, v, ones], axis=0)  # Shape: (3, H, W)

    # Reshape to (3, H*W) for matrix multiplication
    coords_flat = coords.reshape(3, -1)

    # Compute inverse of intrinsic matrix
    K_inv = np.linalg.inv(intrinsic_matrix)

    # Apply K^-1 to pixel coordinates: K^-1 * x
    points_normalized = K_inv @ coords_flat  # Shape: (3, H*W)

    # Multiply by depth: d * K^-1 * x
    depth_flat = depth.reshape(-1)  # Shape: (H*W,)
    points_3d = points_normalized * depth_flat  # Broadcasting: (3, H*W) * (H*W,)

    # Reshape back to (H, W, 3)
    pcloud = points_3d.T.reshape(H, W, 3)
